...
public class Parser {
	...
	public Parser(String fileName) throws IOException {
		...
		for (int i = 0; i < Tokens.size(); i++) {
			if (!Tokens.get(i).getType().equals("EOL")) {
				queue.add(Tokens.get(i));
			}
		}
		Token t = new Token(0, "EOF", "");
		t.setType("EOF");
		t.setValue("");
		queue.add(t);
		 ...
		 * System.out.println(queue.peek()); Token t = queue.peek();
		 * System.out.println(t.toString()); queue.poll(); }
		 */
	...
	private Statement stmtRule() {
		...
		if (first_token != null && first_token.getType().equals("LEFT_CURLY_B")) {
				...
				if (expression != null) {
					...
					if (right_bracket != null && right_bracket.getType().equals("RIGHT_ROUND_B")) {
						...
						if (stmt != null) {
							...
							if (elseStmt != null) {
								return new Statement2(left_bracket, right_bracket, first_token, expression, stmt,
										elseStmt);
							}
						}
					}
			...
			// System.out.println(first_token.getType());
			queue.poll();
			Token left_bracket = queue.peek();
			if (left_bracket != null && left_bracket.getType().equals("LEFT_ROUND_B")) {
				queue.poll();
	...
	private Expression expressionRule() {
		...
		if (expressionDashDash != null) {
			ExpressionBeta expressionBeta = expressionBetaRule();
			if (expressionBeta != null) {
				return new Expression(expressionDashDash, expressionBeta);
			}
		}
		return null;
	}

	private ExpressionBeta expressionBetaRule() {
		ExpressionDash expressionDash = expressionDashRule();
		if (expressionDash != null) {
			ExpressionBeta expressionBeta = expressionBetaRule();
			if (expressionBeta != null) {
				return new ExpressionBeta1(expressionDash, expressionBeta);
			}
		}

		return new ExpressionBeta2(new Epsilon());
	}
	...
	private OP opRule() {
		...
		if (token != null) {
			if (token.getType().equals("AND") || token.getType().equals("LESSTHAN") || token.getType().equals("PLUS")
					|| token.getType().equals("MINUS") || token.getType().equals("MULTIPLY")) {
				queue.poll();
				return new OP(token);
			}
	...
	private Goal goalRule() {
		...
		if (mainClass != null) {
			...
			if (classDeclarations != null) {
				...
				if (eol != null && eol.getType().equals("EOF")) {
					queue.poll();
	...
	private Type typeRule() {
		...
		} else if (type.getType().equals("CHARACTER")) {
			queue.poll();
			Token left = queue.peek();
			if (left.getType().equals("LEFT_SQUARE_B")) {
				queue.poll();
	...
	private AccessSpecifier accessSpecifierRule() {
		// TODO Auto-generated method stub
		Token t = queue.peek();
		if (t.getType().equals("PRIVATE")) {
			queue.poll();
			AccessSpecifier a = new AccessSpecifier1(t);
			return a;
		} else if (t.getType().equals("PUBLIC")) {
			queue.poll();
